...
final class SftpMessageTests: XCTestCase {
	...
	func testValid() {
		...
		var customSink: CustomDemandSink<ByteBuffer, Error>!
		withExtendedLifetime(SftpMessage(
			...
			customSink = CustomDemandSink<ByteBuffer, Error>(
				demand: 2,
			...
			// Send byte 1 of 6. Should fall through to sink.
			var result = message.sendData(ByteBuffer(bytes: [0x01]))
			XCTAssertEqual(result, .success(false))
			XCTAssertEqual(shouldReadHistory, [true])
			XCTAssertEqual(sinkHistory, [[0x01]])

			// Send byte 2 of 6. Should fall through to sink and fill it.
			...
			// Now, sink is full. Increase demand.
			customSink.increaseDemand(4)
			XCTAssertEqual(result, .success(false))
			XCTAssertEqual(shouldReadHistory, [true, false, true])
			XCTAssertEqual(sinkHistory, [[0x01], [0x02]])

			// Finish
			result = message.sendData(ByteBuffer(bytes: [0x03, 0x04, 0x05, 0x06]))
			XCTAssertEqual(result, .success(true))
			XCTAssertEqual(shouldReadHistory, [true, false, true])
			XCTAssertEqual(sinkHistory, [[0x01], [0x02], [0x03, 0x04, 0x05, 0x06]])
...
//	}
	...
	func testInvalidTooManyBytes() {
		...
		var sinkHistory: [[UInt8]] = []
		let customSink = CustomDemandSink<ByteBuffer, Error>(
			demand: 2,
			receiveCompletion: { _ in },
			receiveValue: { sinkHistory.append($0.getBytes(at: 0, length: $0.readableBytes)!) })
		message.data.subscribe(customSink)
