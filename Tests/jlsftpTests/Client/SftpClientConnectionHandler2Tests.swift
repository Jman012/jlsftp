...
final class SftpClientChannelHandler2Tests: XCTestCase {
...
//	}
	...
	public func testWriteHeader() {
		let channel = EmbeddedChannel()
		let handler = SftpClientChannelHandler2(logger: noopLogger)
		XCTAssertNoThrow(try channel.pipeline.addHandler(handler).wait())

		switch handler.state {
		case let .awaitingRequest(requestQueue: queue, canWriteBody: canWriteBody):
			XCTAssert(queue.isEmpty)
			XCTAssertEqual(canWriteBody, false)
		default:
			XCTFail()
		}

		// Write a simple header-only packet
		let requestPacket: Packet = .handleReply(.init(id: 1, handle: "a"))
		let requestMessage = SftpMessage(packet: requestPacket, dataLength: 0, shouldReadHandler: { _ in })
		let request = ClientRequest(message: requestMessage, eventLoop: channel.eventLoop)
		XCTAssertNoThrow(try channel.writeOutbound(request))
		XCTAssertNoThrow(try request.requestMessageSentPromise.futureResult.wait())
		XCTAssertEqual(request.requestSendState, .sent)
