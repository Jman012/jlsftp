...
final class PermissionsV3Tests: XCTestCase {
	...
	func testBinaryRepresentation() {
		for (expBinary, user, group, other, _) in testCases {
			let perm = jlsftp.SftpProtocol.Version_3.PermissionsV3(user: user, group: group, other: other)
	...
	func testToStandard() {
		for (_, user, group, other, expPerm) in testCases {
			let permV3 = jlsftp.SftpProtocol.Version_3.PermissionsV3(user: user, group: group, other: other)
	...
	func testInitFromPermissions() {
		for (_, user, group, other, perm) in testCases {
			let expPermV3 = jlsftp.SftpProtocol.Version_3.PermissionsV3(user: user, group: group, other: other)
