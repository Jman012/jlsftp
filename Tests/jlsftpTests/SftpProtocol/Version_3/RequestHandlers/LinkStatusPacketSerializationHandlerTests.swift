...
final class LinkStatusPacketSerializationHandlerTests: XCTestCase {
	...
	private func getHandler() -> jlsftp.SftpProtocol.Version_3.LinkStatusPacketSerializationHandler {
		return jlsftp.SftpProtocol.Version_3.LinkStatusPacketSerializationHandler()
	}
	...
	func testSerializeValid() {
		let handler = getHandler()
		let packet = LinkStatusPacket(id: 3, path: "a")
		var buffer = ByteBuffer()

		XCTAssertTrue(handler.serialize(packet: .linkStatus(packet), to: &buffer))
