...
final class StatusPacketSerializationHandlerTests: XCTestCase {

	private func getHandler() -> jlsftp.SftpProtocol.Version_3.StatusPacketSerializationHandler {
		return jlsftp.SftpProtocol.Version_3.StatusPacketSerializationHandler()
	}
	...
	func testDeserializeValid() {
		...
		XCTAssertEqual(3, statusPacket.id)
		XCTAssertEqual("a", statusPacket.path)
	}
	...
	func testSerializeValid() {
		let handler = getHandler()
		let packet = StatusPacket(id: 3, path: "a")
		var buffer = ByteBuffer()

		XCTAssertTrue(handler.serialize(packet: .status(packet), to: &buffer))
