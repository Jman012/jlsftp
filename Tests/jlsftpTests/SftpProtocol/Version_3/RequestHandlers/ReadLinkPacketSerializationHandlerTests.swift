...
final class ReadLinkPacketSerializationHandlerTests: XCTestCase {
	...
	private func getHandler() -> jlsftp.SftpProtocol.Version_3.ReadLinkPacketSerializationHandler {
		return jlsftp.SftpProtocol.Version_3.ReadLinkPacketSerializationHandler()
	}
	...
	func testSerializeValid() {
		let handler = getHandler()
		let packet = ReadLinkPacket(id: 3, path: "a")
		var buffer = ByteBuffer()

		XCTAssertTrue(handler.serialize(packet: .readLink(packet), to: &buffer))
