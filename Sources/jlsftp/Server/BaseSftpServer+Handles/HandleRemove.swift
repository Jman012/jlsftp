...
extension BaseSftpServer {
	public func handleRemove(
		...
		if (statResult.st_mode & S_IFREG) == S_IFREG {
			...
			let errorReply: Packet = .statusReply(StatusReplyPacket(id: packet.id, statusCode: .failure, errorMessage: "The filename '\(packet.filename)' is a directory: \(error)", languageTag: "en-US"))
			return replyHandler(SftpMessage(packet: errorReply, dataLength: 0, shouldReadHandler: { _ in }))
		}
		...
		do {
			...
			let errorReply: Packet = .statusReply(StatusReplyPacket(id: packet.id, statusCode: ., errorMessage: "There was an error removing the file '\(packet.filename)': \(error)", languageTag: "en-US"))
			return replyHandler(SftpMessage(packet: errorReply, dataLength: 0, shouldReadHandler: { _ in }))
		}
	}
}
