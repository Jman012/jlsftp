...
public class DemandBridgeSubject<Output, Failure: Error>: Subject {
	...
	public func send(_ value: Output) {
		...
		let additionalDownstreamDemand = downstreamSubscription!.downstream?.receive(value) ?? .none
		downstreamSubscription!.lock.withLock {
			downstreamSubscription!.demand += additionalDownstreamDemand
